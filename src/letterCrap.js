(function () {
	...
	function render(element, image, prev) {
		...
		let words = element.hasAttribute("data-lettercrap-words")
			? element.getAttribute("data-lettercrap-words").split(" ")
			: [];
		let letters = element.hasAttribute("data-lettercrap-letters")
			? element.getAttribute("data-lettercrap-letters")
			: "0101010101_";
		let textCondition =
			prev && prev.width === element.clientWidth && prev.height === element.clientHeight;
		let text = getTextContentWithImageAtSize(
			image,
			element.clientWidth,
